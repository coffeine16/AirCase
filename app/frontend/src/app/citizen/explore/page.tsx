...
export default function ExplorePage() {
  ...
  const wardMedianPm25 = (wardId: string): number => {
    // `pm25` is `number | null` — null means "no value here at this hour", which
    // is NOT zero and must not be averaged in. Bind to a local and narrow with an
    // explicit null check: Number.isFinite() does not narrow `number | null` for
    // TypeScript, so filtering on it alone leaves the nulls in the type AND
    // silently coerces them to 0 in the sort.
    const vals = cells
      .flatMap((c) => {
        const v = c.pm25;
        return c.ward_id === wardId && v != null && Number.isFinite(v) ? [v] : [];
      })
      .sort((a, b) => a - b);
